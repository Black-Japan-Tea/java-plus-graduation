...
import ru.practicum.ewm.request.client.RequestsClient;
...
public class EventServiceImpl implements EventService {
    ...
    public List<ParticipationRequestDto> getRequests(Long userId, Long eventId) {
        ...
        List<ParticipationRequestDto> requests = requestsClient.getByEvent(eventId);
    ...
    public EventRequestStatusUpdateResult updateStatus(Long userId, Long eventId,
        ...
        if (request.getStatus().equals(CONFIRMED)) {
            long confirmed = requests.stream()
                    .filter(req -> CONFIRMED.toString().equals(req.getStatus()))
                    .count();
        ...
        long confirmedRequests = requests.stream()
                .filter(req -> processingRequests.contains(req.getId()) && CONFIRMED.toString().equals(req.getStatus()))
                .count();
        if (confirmedRequests >= 1) {
            throw new DataViolationException("Нельзя изменить статус у уже подтверждённой заявки");
        }
        List<ParticipationRequestDto> participationRequests = requestsClient.updateStatus(
                eventId,
                new RequestStatusUpdateRequest(processingRequests, request.getStatus())
        );
    ...
    public List<EventFullDto> searchEventsByAdmin(Long[] users, String[] states, Integer[] categories,
        ...
        List<EventFullDto> events = eventRepository.findAll(builder, page).getContent().stream()
                ...
                .peek(event -> {
                    Long confirmed = requestsClient.countByEventAndStatus(event.getId(), CONFIRMED);
                    event.setConfirmedRequests(confirmed != null ? confirmed.intValue() : 0);
